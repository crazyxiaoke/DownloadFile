package com.hz.zxk.downloadfile.adapter;
...
import com.hz.zxk.downloadfile.R;
            ...
            public void onClick(final View v) {
                if (apkInfo.getDownloadStatus() == 0) {
                    ...
                    DownloadDispatch.getInstance().download(apkInfo.getUrl(), new DownloadCallback() {
                        ...
                        @Override
                        public void pending() {
                            viewHolder.downloadBtn.setText("等待下载");
                        }
                        ...
                        public void stop(String url) {
                            viewHolder.networkSpeed.setText("停止下载");
                        }

                        @Override
                        public void progress(final long progress, final String networkspeed) {
                            mActivity.runOnUiThread(new Runnable() {
                                @Override
                                public void run() {
                                    viewHolder.progressBar.setProgress((int) progress);
                                    viewHolder.networkSpeed.setText(networkspeed);
                                }
                            });
